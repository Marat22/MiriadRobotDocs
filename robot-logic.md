...
- [РегуляторКоллбеков](#регуляторколлбеков)
  ...
  - [Общий процесс](#общий-процесс)
    ...
    - [6 Обработка коллбеков](#6-обработка-коллбеков)
      - [6.1 Процесс обработки ответов на заявки](#61-процесс-обработки-ответов-на-заявки)
...
Обычно после отправки заявки на неё в течение 5 секунд приходит ответ от брокера (заявка исполнена/не исполнена), однако есть очень маленькая вероятность того, что ответ на заявку от брокера попросту не придёт. Такая ситуация может возникнуть в том случае, если, например, после отправки заявки Робот сразу же потерял соединение с брокером, так что ответ на заявку попросту не был принят. Чтобы понять, что заявка не была исполнена, нужно хранить все отправленные заявки, пока на них не пришёл ответ. Такие заявки хранятся в **списке отправленных заявках**. Функция `проверить_отправленные_заявки()` проверяет, что заявки в **списке отправленных заявках** лежат там не дольше минуты. Если дольше, то они удаляются из **списка отправленных заявок** и затем выводится предупреждение, что ответ на заявку не был получен.
...
- процесс 2 [очистка очереди заявок и снятие заявок с биржи](#22-очистка-очереди-заявок-и-снятие-заявок-с-биржи)
...
sequenceDiagram
  ...
  РегуляторКоллбеков->>Заявки : Вызывает `обработать_ответы_на_заявки()` (процесс 1)
  end
```

Данный процесс включает в себя следующие процессы:
- процесс обработки ответов на заявки

#### 6.1 Процесс обработки ответов на заявки
