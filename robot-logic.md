...
sequenceDiagram
    ...
    FrameworkInterface->>Framework: кладёт заявки в очередь на отправку
...
sequenceDiagram
    ...
    activate ИнтерфейсФреймворка
    ИнтерфейсФреймворка->>Фреймворк: добавляет заявку в очередь заявок на отправку
    deactivate ИнтерфейсФреймворка
...
- Если в аналитику отправляется следующая свеча, то на этот момент ответы на все отправленные аналитикой заявки должны были быть переданы аналитике.
...
очередь_заявок — очередь, в которой хранятся заявки на отправку.
