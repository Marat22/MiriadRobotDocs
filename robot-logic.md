...
sequenceDiagram
    ...
    deactivate ИнтерфейсФреймворка
    Аналитика->>ИнтерфейсФреймворка: передаёт заявку в очередь заявок
    activate ИнтерфейсФреймворка
...
Обычно после отправки заявки на неё в течение 5 секунд приходоит ответ от брокера (заявка исполнена/не исполнена), однако есть очень маленькая вероятность того, что ответ на заявку от брокера попросту не придёт. Такая ситуация может возникнуть в том случае, если, например, после отправки заявки Робот сразу же потерял соединение с брокером, так что ответ на заявку попросту не был принят. Чтобы понять, что заявка не была исполнена, нужно хранить все отправленные заявки, пока на них не пришёл ответ. Такие заявки хранятся в **списке отправленных заявках**. Функция `проверить_отправленные_заявки()` проверяет, что заявки в **списке отправленных заявках** лежат там не дольше минуты. Если дольше, то они удаляются из **списка отправленных заявок** и затем выводится предупреждение, что ответ на заявку не был получен.

Есть вероятность того, что нужно удалить все заявки из очереди (`очистить_очередь_заявок()`) и снять все заявки на бирже (`снять_все_заявки_на_бирже()`). Данные функции необходимы по причине того, что есть требование, что перед отправкой свечей в Аналитику нужно удалить все заявки из очереди и снять все заявки на бирже. 
...
## Общий процесс
В данной секции описаны процессы, происходящие в роботе. Процессы, которые включают в себя несколько других процессов, описаны подробно, другие процессы описаны не так подробно. Например, "[передача свечей аналитике](#передача-свечей-аналитике)" включает в себя множество других процессов и он описан отдельно, а вот "получение свечей с биржи" не включает других процессов и не описан.

Общий процесс содержит в себе все процессы, происходящие в роботе:
```mermaid
sequenceDiagram
  loop Каждую минуту
...
sequenceDiagram
  ...
  Фьючерсы->>Адаптер: Запрашивает информацию о фьючерсах по тикеру
  Адаптер-->>Фьючерсы: Возвращает информацию о фьючерсах по тикеру
  Фьючерс->>Фьючерс: Определяет какой фьючерс сейчас активен
...
sequenceDiagram
    Планировщик->>Свечи: Вызывает `передать_свечи_аналитике()`
    Свечи->>Сценарии: Запрашивает сценарии свечей (процесс 1)
    Сценарии-->>Свечи: Возвращает сценарии свечей (процесс 1)

    Свечи->>Заявки: Вызывает `очистить_очередь_заявок()` (процесс 2)
    Свечи->>Заявки: Вызывает `снять_все_заявки_на_бирже()` (процесс 2)
    Свечи->>Адаптер: Запрашивает свечи (процесс 3)
    Адаптер-->>Свечи: Возвращает сырые свечи (процесс 3)
    Свечи->>ХранилищеБиржевыхДанных: Запрашивает биржевые данные по инструментам свечей (процесс 4)
    ХранилищеБиржевыхДанных-->>Свечи: Возвращает биржевые данные по инструментам свечей (процесс 4)
    Свечи->>Индексы: Запрашивает индекс свечей (процесс 5)
    Индексы-->>Свечи: Возвращает индекс свечей (процесс 5)
    Свечи->>ИнтерфейсАналитики: Передаёт аналитические свечи (процесс 6)
    ИнтерфейсАналитики->>Аналитика: Передаёт аналитические свечи (процесс 6)
```

Данный процесс включает в себя следующие процессы:
- процесс 1 определение сценариев свечей
- процесс 2 очистка очереди заявок и снятие заявок с биржи
- процесс 3 получение свечей с биржи
- процесс 4 получение биржевых данных по фьючерсам свечей
- процесс 5 получение индексов свечей
- процесс 6 [обработка свечей аналитикой](#26-обработка-свечей-аналитикой)
...
sequenceDiagram
    ...
    opt
    Аналитика->>ИнтерфейсФреймворка: вызывает `получить_исторические_свечи()` (процесс 1)
    activate ИнтерфейсФреймворка
    ИнтерфейсФреймворка->>Свечи: запрашивает исторические свечи (процесс 1)
    Свечи-->>ИнтерфейсФреймворка: возвращает исторические свечи (процесс 1)
    ИнтерфейсФреймворка-->>Аналитика: возвращает исторические свечи (процесс 1)
    deactivate ИнтерфейсФреймворка
    end

    opt
    Аналитика->>ИнтерфейсФреймворка: вызывает `получить_стоимость_портфеля()` (процесс 2)
    activate ИнтерфейсФреймворка
    ИнтерфейсФреймворка->>Адаптер: запрашивает стоимость портфеля (процесс 2)
    Адаптер-->>ИнтерфейсФреймворка: возвращает стоимость портфеля (процесс 2)
    ИнтерфейсФреймворка-->>Аналитика: возвращает текущую стоимость портфеля (процесс 2)
    deactivate ИнтерфейсФреймворка
    end

    opt Аналитика решила отправить заявку
    Аналитика->>ИнтерфейсФреймворка: передаёт заявку в очередь заявок (процесс 3)
    activate ИнтерфейсФреймворка
    ИнтерфейсФреймворка->>Заявки: добавляет заявку в очередь заявок на отправку (процесс 3)
    end
...
Данный процесс включает в себя следующие процессы:
- процесс 1 получение исторических свечей
- процесс 2 получение стоимости портфеля
- процесс 3 добавление заявки в очередь заявок
  - **обратите внимание**, здесь заявки не отправляются, а именно добавляются в очередь на отправку. Сама отправка заявок происходит только в процессе [отправки заявок из очереди](#3-отправка-заявок-из-очереди), который входит в [основной процесс](#общий-процесс) робота.
