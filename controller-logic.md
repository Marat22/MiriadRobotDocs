...
Фреймворк должен быть реализован через ООП

Почему ООП?

Почему ООП?- Фреймворк должен уметь торговать на разных рынках и через разное API. С помощью ООП было бы унифицировать поведение для всех рынков и API. Например, используя абстрактные классы и наследование.
...
### ИнтерфейсАналитики
— класс, через который контроллер взаимодействует с аналитикой. 

```mermaid
classDiagram
  class ИнтерфейсАналитики{
    +отправить_свечи(аналитические свечи) None
    +получить_аналитические_позиции(тикер) количество позиций
  }

```
Описание интерфейса:
- +отправить_свечи() — передаёт аналитике [аналитические свечи](#термины) для выполнения сценариев на них.
- +получить_аналитические_позиции() — возвращает текущее количество позиций в аналитике.
...
classDiagram
  class ИнтерфейсФреймворка{
    ...
    +получить_состояние_счёта() состояние счёта
  }

```

Описание интерфейса:
- получить_исторические_свечи() — возвращает исторические свечи за указанный период
- получить_состояние_счёта() — возвращает текущее состояние счёта
